...
void PolySet::setWeight(const std::vector<std::string>& bones, const std::vector<float>& weights)
{
  bone_names_array = {bones};
  bone_weights_array = {weights};
  weight_indices.assign(indices.size(), 0);
}
